<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Registrar Causa de Muerte (No Tránsito)</h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 md:p-8">
                    ...
                    <form action="{{ route('causa_muerte.no_transito.store') }}" method="POST" x-data="formData()">
                        @csrf
                        ...
                        <div class="space-y-10">
                            ...
                            <section>
                                <h3 class="text-lg font-medium text-gray-900 border-b pb-2 mb-4">Datos Generales de
                                    Ingreso</h3>
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    <div>
                                        <label for="numero_entrada"
                                            class="block text-sm font-medium text-gray-700">Número de Entrada</label>
                                        <input type="text" name="numero_entrada" id="numero_entrada"
                                            value="{{ old('numero_entrada') }}"
                                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                                    </div>
                                    <div>
                                        <label for="numero_expediente"
                                            class="block text-sm font-medium text-gray-700">Número de Expediente</label>
                                    ...
                                    <div>
                                        <label for="despacho"
                                            class="block text-sm font-medium text-gray-700">Despacho</label>
                                        <select name="despacho" id="despacho"
                                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                            @foreach (['SENAMECF', 'CICPC', 'CPNB', 'GNB', 'OTROS'] as $option)
                                                <option value="{{ $option }}"
                                                    {{ old('despacho') == $option ? 'selected' : '' }}>
                                                    {{ $option }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div>
                                        <label for="fecha_ingreso_cadaver"
                                            class="block text-sm font-medium text-gray-700">Fecha de Ingreso del
                                            Cadáver</label>
                                        <input type="date" name="fecha_ingreso_cadaver" id="fecha_ingreso_cadaver"
                                            value="{{ old('fecha_ingreso_cadaver', date('Y-m-d')) }}"
                                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                                    </div>
                                    <div>
                            ...
                            <section>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    <div>
                                        <label for="delito"
                                            class="block text-sm font-medium text-gray-700">Delito</label>
                                        <select name="delito" id="delito"
                                            ...
                                            @foreach (['HOMICIDIO', 'SUICIDIO', 'INTERVENCIÓN LEGAL', 'A DETERMINAR', 'RESPONSABILIDAD PROFESIONAL', 'NATURAL', 'ACCIDENTAL'] as $option)
                                                <option value="{{ $option }}"
                                                    {{ old('delito') == $option ? 'selected' : '' }}>
                                                    {{ $option }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="md:col-span-2">
                                        <label for="causa_muerte" class="block text-sm font-medium text-gray-700">Causa
                                            de Muerte Presunta</label>
                                        <select name="causa_muerte" id="causa_muerte"
                                            ...
                                            @foreach (['HXAF', 'HXAB', 'A DETERMINAR', 'AHORCADO', 'POLITRAUMATISMO', 'NATURAL', 'AHOGADO', 'QUEMADO', 'ELECTROCUTADO', 'HIPOTERMÍA', 'ASFIXIA MÉCANICA', 'ESTRANGULAMIENTO', 'ENVENENAMIENTO'] as $option)
                                                <option value="{{ $option }}"
                                                    {{ old('causa_muerte') == $option ? 'selected' : '' }}>
                                                    {{ $option }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="md:col-span-3">
                                        <label for="mecanismo_causa_muerte"
                                            class="block text-sm font-medium text-gray-700">Mecanismo de la Causa de
                                            Muerte</label>
                                        <select name="mecanismo_causa_muerte" id="mecanismo_causa_muerte"
                                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                            @foreach (['CAÍDA DE ALTURA', 'CAÍDA DE SUS PROPIOS PIES', 'OBJETO CONTUNDENTE', 'ARROLLAMIENTO DE METRO', 'LINCHAMIENTO', 'INHALACIÓN DE GASES', 'TRAUMA TÉRMICO', 'APLASTAMIENTO', 'TERREMOTO', 'ACCIDENTE AÉREO', 'INUNDACIÓN', 'RAYOS', 'INCENDIO', 'INTOXICACIÓN EXÓGENA', 'EXPLOSIÓN', 'ACCIDENTE DE TRÁNSITO', 'ABALANZAMIENTO', 'TAPIADO', 'SE DESCONOCE', 'ASFIXIA MÉCANICA', 'OTROS'] as $option)
                                                <option value="{{ $option }}"
                                                    {{ old('mecanismo_causa_muerte') == $option ? 'selected' : '' }}>
                                                    {{ $option }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                            ...
                            <section id="location-wrapper">
                                <h3 class="text-lg font-medium text-gray-900 border-b pb-2 mb-4">Datos de Locación del
                                    Suceso</h3>
                                <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                                    ...
                                    <div>
                                        <label for="parroquia_id"
                                            class="block text-sm font-medium text-gray-700">Parroquia</label>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                                    <div>
                                        <label for="direccion_remocion_cadaver"
                                            class="block text-sm font-medium text-gray-700">Lugar de Remoción</label>
                                        <select name="direccion_remocion_cadaver" id="direccion_remocion_cadaver"
                                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                            @foreach (['AMBULATORIO', 'CDI', 'CENTRO PENITENCIARIO', 'CICPC', 'CLINICA', 'HOSPITAL', 'NO DECLARADO', 'OTROS', 'CPNB', 'GNB', 'VIA PUBLICA'] as $option)
                                                <option value="{{ $option }}"
                                                    {{ old('direccion_remocion_cadaver') == $option ? 'selected' : '' }}>
                                                    {{ $option }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div>
                                        <label for="categorizacion_referencias"
                                            class="block text-sm font-medium text-gray-700">Categorización del
                                            Sitio</label>
                                        <select name="categorizacion_referencias" id="categorizacion_referencias"
                                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                            @foreach (['BARRIO O CASERÍO', 'CENTRO DE SALUD', 'CENTRO PENITENCIARIO', 'GRAN MISIÓN VIVIENDA VENEZUELA (GMVV)', 'HOTEL O POSADA', 'INSTALACIONES DEL ESTADO VENEZOLANO', 'INTERIOR DE VEHÍCULO', 'URBANIZACIÓN O CONJUNTO RESIDENCIAL', 'VÍA PÚBLICA', 'OTROS'] as $option)
                                                <option value="{{ $option }}"
                                                    {{ old('categorizacion_referencias') == $option ? 'selected' : '' }}>
                                                    {{ $option }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="md:col-span-2">
                                        <label for="direccion_exacta"
                                            class="block text-sm font-medium text-gray-700">Dirección Exacta</label>
                                        <input type="text" name="direccion_exacta" id="direccion_exacta"
                                            value="{{ old('direccion_exacta') }}"
                                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                                    </div>
                                </div>
                            </section>
                            ...
                            <section>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    <div class="md:col-span-3">
                                        <label for="nombres_apellidos"
                                            class="block text-sm font-medium text-gray-700">Nombres y Apellidos</label>
                                    ...
                                    <div class="grid grid-cols-3 gap-4">
                                        <div class="col-span-2">
                                            ...
                                            <input type="number" name="edad" id="edad" x-model.lazy="edad"
                                                @input="validarEdad" value="{{ old('edad') }}"
                                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm"
                                                required>
                                        </div>
                                        <div>
                                            <label for="edad_medida"
                                                class="block text-sm font-medium text-gray-700">Medida</label>
                                            <select name="edad_medida" id="edad_medida" x-model="medida"
                                                @change="calcularGrupoEtario"
                                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm"
                                                required>
                                                @foreach (['AÑO (S)', 'MES (ES)', 'DÍA (S)', 'MES (ES) DE GESTACIÓN', 'SE DESCONOCE'] as $option)
                                                    <option value="{{ $option }}"
                                                        {{ old('edad_medida', 'AÑO (S)') == $option ? 'selected' : '' }}>
                                    ...
                                    <div>
                                        ...
                                        <select name="sexo" id="sexo" x-model="sexo"
                                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                                            @foreach (['MASCULINO', 'FEMENINO', 'POR DETERMINAR', 'SE DESCONOCE'] as $option)
                                                <option value="{{ $option }}"
                                                    {{ old('sexo') == $option ? 'selected' : '' }}>{{ $option }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div x-show="sexo === 'FEMENINO'" style="display: none;">
                                        <label for="embarazada"
                                            class="block text-sm font-medium text-gray-700">¿Embarazada?</label>
                                    ...
                                    <div>
                                        ...
                                        <select name="nacionalidad" id="nacionalidad"
                                            ...
                                            @foreach (['VENEZOLANA', 'EXTRANJERA', 'POR DETERMINAR', 'SE DESCONOCE'] as $option)
                                                <option value="{{ $option }}"
                                                    {{ old('nacionalidad') == $option ? 'selected' : '' }}>
                                                    {{ $option }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div>
                                        ...
                                        <select name="tipo_identificacion" id="tipo_identificacion"
                                            ...
                                            @foreach (['CÉDULA', 'PASAPORTE', 'PARTIDA DE NACIMIENTO', 'NO IDENTIFICADO'] as $option)
                                                <option value="{{ $option }}"
                                                    {{ old('tipo_identificacion') == $option ? 'selected' : '' }}>
                                                    {{ $option }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="md:col-span-2">
                                        <label for="sitio_donde_laboraba"
                                            class="block text-sm font-medium text-gray-700">Sitio Donde
                                            Laboraba</label>
                                        <select name="sitio_donde_laboraba" id="sitio_donde_laboraba"
                                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                            @foreach (['CUERPO DE SEGURIDAD CIUDADANA', 'SERVIDOR PÚBLICO', 'EMPRESA PRIVADA', 'DETENIDO'] as $option)
                                                <option value="{{ $option }}"
                                                    {{ old('sitio_donde_laboraba') == $option ? 'selected' : '' }}>
                                                    {{ $option }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div>
                                        ...
                                        <select name="nivel_instruccion" id="nivel_instruccion"
                                            ...
                                            @foreach (['SIN ESTUDIOS', 'BÁSICA', 'BACHILLER', 'UNIVERSITARIO', 'SIN INFORMACIÓN'] as $option)
                                                <option value="{{ $option }}"
                                                    {{ old('nivel_instruccion') == $option ? 'selected' : '' }}>
                                                    {{ $option }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="md:col-span-3">
                                        <label for="descripcion"
                                            class="block text-sm font-medium text-gray-700">Descripción de Vestimentas
                                            y Pertinencias</label>
                            ...
                            <section>
                                <h3 class="text-lg font-medium text-gray-900 border-b pb-2 mb-4">Datos Finales y
                                    Observaciones</h3>
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    <div>
                                        <label for="fecha_dictamen_muerte"
                                            class="block text-sm font-medium text-gray-700">Fecha Dictamen de
                                            Muerte</label>
                                        <input type="date" name="fecha_dictamen_muerte" id="fecha_dictamen_muerte"
                                            value="{{ old('fecha_dictamen_muerte') }}"
                                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                                        @error('fecha_dictamen_muerte')
                                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    <div>
                                        <label for="hora_dictamen_muerte"
                                            class="block text-sm font-medium text-gray-700">Hora Dictamen de
                                            Muerte</label>
                                        <input type="time" name="hora_dictamen_muerte" id="hora_dictamen_muerte"
                                            value="{{ old('hora_dictamen_muerte') }}"
                                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                                        @error('hora_dictamen_muerte')
                                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    <div>
                                        <label for="fases_descomposicion"
                                            class="block text-sm font-medium text-gray-700">Fase de
                                            Descomposición</label>
                                        <select name="fases_descomposicion" id="fases_descomposicion"
                                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                            @foreach (['FRESCO', 'PUTREFACTO', 'OSAMENTO'] as $option)
                                                <option value="{{ $option }}"
                                                    {{ old('fases_descomposicion') == $option ? 'selected' : '' }}>
                                                    {{ $option }}</option>
                                            @endforeach
                                        </select>
                                        @error('fases_descomposicion')
                                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    <div class="md:col-span-3">
                            ...
                            <div class="flex justify-end space-x-4 pt-4 border-t">
                                <a href="{{ route('causa_muerte.transito.index') }}"
                                    class="px-4 py-2 border rounded-md text-sm">Cancelar</a>
                                <button type="submit"
                                    class="px-4 py-2 bg-blue-600 text-white rounded-md text-sm">Guardar
                                    Registro</button>
                            </div>
                        </div>
                    </form>
